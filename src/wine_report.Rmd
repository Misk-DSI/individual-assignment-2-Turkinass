...
ggcorr(cor(winequality),  # make a correlation table
       ...
       label_size = 3,
       hjust = 0.8)       # move titles horizontally
```
...
In conclusion we found that the red wine quality dataset does not have enough variation in its samples when it comes to `quality`, with that in mind, the most influencing variables on its quality are `sulphates`, `alcohol` and `volatile.acidity` (negatively).
